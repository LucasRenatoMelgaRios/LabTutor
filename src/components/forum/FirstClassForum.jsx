...
export const FirstClassForum = () => {
    ...
    const [acquiredEmojis, setAcquiredEmojis] = useState([]); // Estado para emojis adquiridos
    ...
    const API_USER_URL = "https://66ca61e159f4350f064f0e88.mockapi.io/api/labtutor/users"; // URL para usuarios
    ...
    useEffect(() => {
      const fetchUserData = async () => {
        try {
          const response = await axios.get(`${API_USER_URL}/${user.id}`); // Suponiendo que `id` es el ID del usuario
          const userEmojis = response.data.emojisComprados; // Obtiene los emojis comprados
          setAcquiredEmojis(userEmojis); // Almacena en el estado
        } catch (error) {
      ...
      fetchUserData();
    }, [user.id]); // Ejecuta al cambiar el ID del usuario
    ...
    const onCustomEmojiClick = (emoji) => {
      ...
      inputRef.current.innerHTML += imgTag; // Agregar la etiqueta de imagen directamente
    ...
    const handleInputChange = () => {
      ...
      customEmojis.forEach(emoji => {
        ...
        updatedInput = updatedInput.replace(regex, imgTag); // Reemplazar el texto del emoji por la etiqueta de imagen
      });
  
      inputRef.current.innerHTML = updatedInput; // Actualizar el input
